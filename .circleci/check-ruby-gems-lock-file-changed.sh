...
# TODO feat/revamp whitelisting is temporary
# remember to *remove* after merging
if [ "${CIRCLE_BRANCH#proj/}" != "${CIRCLE_BRANCH}" -o "${CIRCLE_BRANCH#dependabot/}" != "${CIRCLE_BRANCH}" -o "${CIRCLE_BRANCH#feat/revamp}" != "${CIRCLE_BRANCH}" -o "${LOCKFILE_DIFF_SIZE}" -eq 0 ] ; then
