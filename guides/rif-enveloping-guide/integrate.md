...
The simplest option to use Enveloping in your wallet or dApp is by calling the Relay Server directly. See the instructions for running a Relayer in [Getting Started](/x-1Hu95ZTDGPmcvzZfVSgA). The communication with the Relay Server is through HTTP requests.
...
As we mentioned in the [documentation](/@missowans/B1HZMAx4_), an advantage of the Enveloping's solution is the chance to have a token's wallet without deploying it. When a user needs to use her tokens, she needs to deploy the smart wallet using a deploy request. Thereby, when a gas-less account sent a transaction through Enveloping, they could use their smart wallet address to pay for the gas.
