...
- A [liquidity provider server (LPS)](/guides/flyover/tools#liquidity-provider-server-lps) instance
...
2. Make a call to LBC's `hashQuote`, which accepts a quote object and returns its hash.
...
For peg-ins the following fields should be specified in the call to the [LBC](https://github.com/rsksmart/liquidity-bridge-contract) in the method: `registerPegIn()`:
...
curl \
--location \
--request POST 'http://flyover-01.aws-us-east-2.testnet.dev.flyover.rskcomputing.net:8080/getQuote' \
...
Users can invoke the `eth_call` JSON-RPC directly, or by using a web3 library.
...
```
curl --location --request POST 'http://flyover-01.aws-us-east-2.testnet.dev.flyover.rskcomputing.net:8080/acceptQuote' \
--header 'Content-Type: application/json' \
--data-raw '{
