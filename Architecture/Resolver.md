...
Along the RNS registry and initial registrar there is a public multichain resolver deployed as well (see the MultiChainResolver contract). This contract is available for anybody to use it, but the only restriction is that only owners of a domain can modify its record in this resolver.
