...
## Cache
The information presented in the UI comes from **RIF Marketplace Cach**e**. This component is permanently listening to the blockchain and populating a read-only database with the information from the smart contracts organized in a readable and optimized way. Its main objective is to improve the performance, scalability, and overall user experience of the RIF Marketplace. It is important to note the Cache can be reconstructed at any time from the information available in the Blockchain, and works only in read-only mode providing information to the UI but in now way modifying or altering any information on the smart contracts.
