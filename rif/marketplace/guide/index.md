---
layout: rsk
title: "RIF Marketplace User Guide"
tags: rif, marketplace, guide
description: "RIF Marketplace User Guide"
---

- [**Introduction**](/rif/marketplace/guide/intro/)
    - [Use the RIF Marketplace](/rif/marketplace/guide/intro/#use-the-rif-marketplace)
- [**Setup**](/rif/marketplace/guide/setup/)
    - [**Compatible wallets**](/rif/marketplace/guide/setup/#compatible-wallets)
        - [Connect Nifty wallet to RSK](/rif/marketplace/guide/setup/#connect-nifty-wallet-to-rsk)
        - [Connect Metamask wallet to RSK](/rif/marketplace/guide/setup/#connect-metamask-wallet-to-rsk)
        - [Connect Brave to RSK](/rif/rns/guide/brave/)
    - [Getting RBTC](/rif/marketplace/guide/setup/#getting-rbtc)
    - [**Getting RIF Tokens**](/rif/marketplace/guide/setup/#getting-rif-tokens)
        - [Display RIF Token in Nifty wallet](/rif/marketplace/guide/setup/#display-amount-of-tokens-in-the-nifty-wallet)
        - [Display RIF Token in Metamask wallet](/rif/marketplace/guide/setup/#display-amount-of-tokens-in-the-metamask-wallet)
        - [Display RIF Token in Brave wallet](/rif/rns/guide/brave/#add-the-rif-token)
- [**Getting started**](/rif/marketplace/guide/getting-started/)
    - [Buy a domain](/rif/marketplace/guide/getting-started/#buy-a-domain)
    - [Sell a domain](/rif/marketplace/guide/getting-started/#sell-a-domain)
    - [Cancel a listed domain](/rif/marketplace/guide/getting-started/#cancel-a-domain)

----

[Github](https://github.com/rsksmart?q=rif-marketplace) |
[Access the RIF Marketplace](https://marketplace.testnet.rifos.org)
